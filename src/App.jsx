...
import { Activity, BookOpen, Boxes, BrainCircuit, CircuitBoard, FlaskConical, Gauge, GitBranch, GraduationCap, LayoutDashboard, Menu, NotebookPen, Plus, Settings, ShieldAlert, Sparkles, Target, TestTube2, X, Zap } from 'lucide-react'

const nav = [
  ['/', 'Dashboard', LayoutDashboard], ['/curriculum', 'Curriculum', GraduationCap], ['/notes', 'Notes', NotebookPen], ['/tests', 'Tests', TestTube2], ['/circuits', 'Circuits', CircuitBoard], ['/components', 'Components', Boxes], ['/projects', 'Projects', GitBranch], ['/mistakes', 'Mistakes', ShieldAlert], ['/analytics', 'Analytics', Activity], ['/wellness', 'Wellness', Sparkles],
]
...
const stats = [
  ['Engineering Progress', '57%', '+8.4%', Gauge], ['Lessons Mastered', '18', '+4 this month', GraduationCap], ['Study Hours', '42.6', '+6.2h', BookOpen], ['Projects', '7', '2 active', GitBranch],
]

const subjects = [
  ['Circuit Analysis', 72, '12 / 17 lessons'], ['Embedded Systems', 48, '8 / 17 lessons'], ['Digital Logic', 61, '11 / 18 lessons'], ['Programming', 54, '14 / 26 lessons'],
]
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      <header className="topbar"><button className="icon-btn menu-btn" onClick={() => setOpen(true)}><Menu size={20}/></button><div className="crumb"><span>ENGINEERING LAB</span><b>/</b><strong>{pageTitle.toUpperCase()}</strong></div><div className="top-actions"><span className="clock mono">{status}</span><button className="quick-btn" onClick={() => setQuick(!quick)}><Plus size={17}/> QUICK ACTION</button></div></header>
      ...
      {path === '/' ? <Dashboard /> : <ModulePage title={pageTitle} Icon={Icon} description={module?.[1] ?? 'Engineering command center.'} />}
    </main>
  </div>
}
